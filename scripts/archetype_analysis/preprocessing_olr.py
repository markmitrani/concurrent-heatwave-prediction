...
def pick_summer_months(ds):
    ...
    summer_ds = ds.sel(time=ds['time'].dt.month.isin([5, 6, 7, 8]))
    return summer_ds
...
def change_lon_format(ds, var):
    ds = ds.copy()
    new_lon = ((ds['lon'] + 180) % 360) - 180
    ds = ds.assign_coords(lon=new_lon)
    ds = ds.sortby('lon')  # re-sort data along new longitude values
    return ds
...
def sanity_check(ds, var):
    ...
    print(f"\tOLR range: [{ds[var].min().values},{ds[var].max().values}], avg: {ds[var].mean().values}\n")

def main():
    filename_incl_path = "/scistor/ivm/data_catalogue/climate_models/lentis/day/toa/toa_day_equator_ensemble101.nc"
    output_filename = "lentis_toa_JJA_2deg_101_deseason.nc"

    varname = 'rlut'

    dataset = xr.open_dataset(filename_incl_path)
    print("=== Original dataset: ===")
    sanity_check(dataset, varname)
    print(dataset[varname].isel(time=0).sel(lat=24, lon = 30, method='nearest').item())
    print(dataset[varname].isel(time=0).sel(lat=24, lon = 300, method='nearest').item())
    print(dataset[varname].isel(time=0).sel(lat=24, lon = 210, method='nearest').item())
    
    dataset = change_lon_format(dataset, varname)
    print(dataset[varname].isel(time=0).sel(lat=24, lon = 30, method='nearest').item())
    print(dataset[varname].isel(time=0).sel(lat=24, lon = -60, method='nearest').item())
    print(dataset[varname].isel(time=0).sel(lat=24, lon = -150, method='nearest').item())
    ...
    sanity_check(dataset, varname)
    print(dataset[varname].lat.values)
    print(dataset[varname].lat.values[:-1])
    print(dataset[varname].lat.values[12:-1])

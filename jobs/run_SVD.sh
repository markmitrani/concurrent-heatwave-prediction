...
#SBATCH --output=svd-%j.log    # Output to a log file
...
cp "${REPO_DIR}${DATA_DIR}/${STREAM_FILE_AA}" $TMPDIR/

# Copy style file
cp ${REPO_DIR}${SCRIPT_PRED_DIR}/*.mplstyle $TMPDIR/
...
cp ${SVD_FILE} ${REPO_DIR}${DATA_DIR_OUT}/
cp ${ZMAP_FILE} ${REPO_DIR}${DATA_DIR_OUT}/
